...
fn write_operation(
	...
	let operation_responses: Result<Vec<_>, _> =
		...
		.map(|(&status_code, schema)| {
			...
			Ok((http_status_code, variant_name, schema.as_ref()))
		})
		.collect();
	...
	let has_string_response =
		operation_responses.iter().any(|&(_, _, schema)| {
			if let Some(swagger20::Schema { kind: swagger20::SchemaKind::Ty(swagger20::Type::String { .. }), .. }) = schema {
				true
			}
			else {
				false
	...
	if has_string_response {
		writeln!(file, "pub enum {}<'a> {{", operation_result_name)?;
	}
	else {
	...
	for &(_, variant_name, schema) in &operation_responses {
		if let Some(schema) = schema {
			...
			else if let swagger20::SchemaKind::Ty(swagger20::Type::String { .. }) = schema.kind {
				writeln!(file, "    {}(&'a str),", variant_name)?;
			}
			else {
	...
	let uses_buf =
		operation_responses.iter().any(|&(_, _, schema)| match schema.map(|schema| &schema.kind) {
			Some(swagger20::SchemaKind::Ty(swagger20::Type::String { .. })) |
			Some(swagger20::SchemaKind::Ref(_)) => true,

			_ => false,
		});

	if has_string_response {
		writeln!(file, "impl<'a> ::Response<'a> for {}<'a> {{", operation_result_name)?;
	}
	else {
		writeln!(file, "impl<'a> ::Response<'a> for {} {{", operation_result_name)?;
	}

	if uses_buf {
		writeln!(file, "    fn try_from_slice(status_code: ::http::StatusCode, buf: &'a [u8]) -> Result<Self, ::ResponseError> {{")?;
	}
	else {
		writeln!(file, "    fn try_from_slice(status_code: ::http::StatusCode, _: &'a [u8]) -> Result<Self, ::ResponseError> {{")?;
	}

	writeln!(file, "        Ok(match status_code {{")?;
		...
		if let Some(schema) = schema {
			...
			match &schema.kind {
				...
				swagger20::SchemaKind::Ref(_) => if is_watch {
					...
					writeln!(file, "                }};")?;
					writeln!(file, "                {}::{}(result, byte_offset)", operation_result_name, variant_name)?;
				}
				else {
			...
			writeln!(file, "{}::{},", operation_result_name, variant_name)?;
		}
	}
	writeln!(file, "            _ => {}::Other,", operation_result_name)?;
	writeln!(file, "        }})")?;
	writeln!(file, "    }}")?;
	writeln!(file, "}}")?;
