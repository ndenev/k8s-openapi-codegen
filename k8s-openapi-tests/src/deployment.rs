...
fn list() {
	...
	let deployment_list =
		::get_single_value(response, |response, _, _| match response {
			#[cfg(feature = "v1_7")] Ok(apps::ListAppsV1beta1NamespacedDeploymentResponse::Ok(deployment_list)) =>
				Ok(::SingleValueResult::GotValue(deployment_list)),
			#[cfg(feature = "v1_8")] Ok(apps::ListAppsV1beta2NamespacedDeploymentResponse::Ok(deployment_list)) =>
				Ok(::SingleValueResult::GotValue(deployment_list)),
			#[cfg(not(any(feature = "v1_7", feature = "v1_8")))] Ok(apps::ListAppsV1NamespacedDeploymentResponse::Ok(deployment_list)) =>
				Ok(::SingleValueResult::GotValue(deployment_list)),
