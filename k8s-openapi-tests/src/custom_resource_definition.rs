...
fn list() {
	...
	let custom_resource_definition: apiextensions::CustomResourceDefinition =
		::get_single_value(response, |response, _status_code, _response_body| match response {
			#[cfg(feature = "v1_8")] Ok(apiextensions::CreateApiextensionsV1beta1CustomResourceDefinitionResponse::Other) if _status_code == ::http::StatusCode::CREATED =>
				match ::serde_json::from_slice(_response_body) {
					Ok(custom_resource_definition) => Ok(::SingleValueResult::GotValue(custom_resource_definition)),
					Err(ref err) if err.is_eof() => Ok(::SingleValueResult::NeedMoreData),
					Err(err) => Err(err.into()),
				},
			#[cfg(not(feature = "v1_8"))] Ok(apiextensions::CreateApiextensionsV1beta1CustomResourceDefinitionResponse::Created(custom_resource_definition)) =>
				Ok(::SingleValueResult::GotValue(custom_resource_definition)),
